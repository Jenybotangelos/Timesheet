...
export default function ProjectView() {
  ...
  function getStatusLabel(bucket: BucketData) {
    if (bucket.completed)
      return { text: "Completed", color: "text-green-400 bg-green-500/20 border-green-500/30" };
    if (bucket.inProgress)
      return { text: "In Progress", color: "text-orange-400 bg-orange-500/20 border-orange-500/30" };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1c2e] via-[#16213e] to-[#0f3460]">
      {/* Header */}
      <div className="bg-white/10 backdrop-blur-md border-b border-white/10 px-4 sm:px-6 py-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          <button onClick={() => navigate("/projects")} className="text-[#4fc3f7] hover:text-white transition-colors">
            ← Back
          </button>
          <h1 className="text-lg sm:text-xl font-semibold text-white">{projectName}</h1>
        </div>
      </div>
          ...
          <div className="space-y-4">
            {tasks.map((task) => (
              ...
              >
                {/* Task Header */}
                <div
                  className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-5 py-4 cursor-pointer hover:bg-white/5 transition-all"
                  onClick={() => toggleTask(task.id)}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-white/50 text-sm">{task.expanded ? "▼" : "▶"}</span>
                    <h3 className="text-white font-semibold text-base sm:text-lg">{task.name}</h3>
                  </div>
                  {/* Progress summary */}
                  <div className="flex items-center gap-2">
                    {(() => {
                      ...
                      const allDone =
                        taskStages.length > 0 && taskStages.every((b) => task.buckets[b]?.completed);
                      if (allDone) {
                        return (
                          <span className="text-green-400 text-xs font-medium px-2 py-1 bg-green-500/15 border border-green-500/30 rounded-full">
                            ✓ All Completed
                          </span>
                        );
                      }
                      if (activeBucket) {
                        return (
                          <span className="text-orange-400 text-xs font-medium px-2 py-1 bg-orange-500/15 border border-orange-500/30 rounded-full">
                            ● {activeBucket}
                          </span>
                        );
                      }
                      if (taskStages.length === 0) {
                        return (
                          <span className="text-white/40 text-xs font-medium px-2 py-1 bg-white/5 border border-white/15 rounded-full">
                            No Stages
                          </span>
                        );
                      }
                      ...
                      {BUCKETS.filter((b) => task.buckets[b]).map((bucket) => {
                        ...
                        return (
                          <div
                            ...
                            title={`${bucket}: ${
                              b?.completed ? "Done" : b?.inProgress ? "Active" : "Pending"
                            }`}
                          />
                        );
                      })}
                          ...
                          {data.assignedTo.length > 0 && (
                            <div className="mt-2">
                              ...
                              <span className="text-[#4fc3f7] text-xs">
                                {data.assignedTo
                                  .map((email) => employees[email] || email.split("@")[0])
                                  .join(", ")}
                              </span>
                            </div>
                          )}
                            ...
                            <div className="mt-2">
                              ...
                              <ul className="mt-1 space-y-0.5">
                                {data.acceptanceCriteria
                                  .filter((c) => c.trim())
                                  .map((c, i) => (
                                    <li key={i} className="text-white/70 text-xs pl-3">
                                      • {c}
                                    </li>
                                  ))}
                              </ul>
